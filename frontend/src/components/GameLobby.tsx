...
function GameLobby(props: any) {
    ...
    const onNewGame = async () => {
        setLoading(true);
        const gameAddResult = await gamesRef.add(newGameInfo(
               currentUser,
        ...
        if (game) {
            ...
            if (!currentPlayers.find((pId: any) => pId === currentUser)) {
                await gamesRef.doc(gameId).set(
                    {...data, player_ids: [...currentPlayers, currentUser]}
                );
            } else {
    ...
    return (
    currentGame ?
     <div><p>Already in a game! Rejoining.</p></div> :
     <div>
        Active Games
        {loading && 'Loading'}
        {games.map(g => {
        ...
        <button onClick={onNewGame} disabled={loading}>New Game</button>
