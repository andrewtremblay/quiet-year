...
const shuffleCards = (cards: Card[]) =>
  cards
    .map((c) => ({ sortVal: Math.random(), card: c }))
    .sort((a, b) => a.sortVal - b.sortVal)
    .map((cv) => cv.card);

export const cardFromCardIcon = (cardIcon: string): Card => {
  const found = RawCards.find((c) => c.icon === cardIcon) as Card;
  if (!found) {
    throw "cannot find card by icon: '" + cardIcon + "'";
  }
  return found;
};
